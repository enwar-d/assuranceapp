...
final List<TabItem> kTabs = [
  const TabItem(label: 'Home',    url: '$kBase/my-account/dashboard',       match: 'dashboard'),
  const TabItem(label: 'Usage',   url: '$kBase/my-account/usage-history',   match: 'usage'),
  const TabItem(label: 'Add-Ons', url: '$kBase/my-account/select-services', match: 'select-services'),
  const TabItem(label: 'Alerts',  url: '$kBase/my-account/notifications',   match: 'notification'),
  const TabItem(label: 'Profile', url: '$kBase/my-account/profile',         match: 'profile'),
];
...
class _NavPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: kCyan.withValues(alpha: 0.82),
          borderRadius: BorderRadius.circular(999),
        ),
...
class _NavBtnState extends State<_NavBtn> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    final iconColor = widget.active ? Colors.white : Colors.white.withValues(alpha: 0.45);
    return GestureDetector(
      ...
      child: ScaleTransition(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: widget.active ? kPurple.withValues(alpha: 0.85) : Colors.transparent,
            borderRadius: BorderRadius.circular(999),
          ),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  color: widget.active ? Colors.white : Colors.white.withValues(alpha: 0.4),
                  letterSpacing: 0.2,
                ),
              ),
            ],
    ...
    return SizedBox(
      ...
      child: Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.signal_cellular_alt, color: kCyan, size: 40),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color:        dark ? kCyan.withValues(alpha: 0.1) : kCyan.withValues(alpha: 0.08),
          borderRadius: BorderRadius.circular(999),
          border:       Border.all(color: dark ? kCyan.withValues(alpha: 0.2) : kCyan.withValues(alpha: 0.25)),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color: dark ? Colors.white.withValues(alpha: 0.55) : Colors.black.withValues(alpha: 0.5),
              ),
            ),
          ],
        ),
        ...
        child: widget.primary
            ? Container(
                ...
                decoration: BoxDecoration(
                  color: kCyan,
                  borderRadius: BorderRadius.circular(999),
                  boxShadow: [BoxShadow(color: kCyan.withValues(alpha: 0.35), offset: const Offset(0, 6), blurRadius: 14)],
                ),
            ...
            : Container(
                ...
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(999),
                  border: Border.all(
                    color: widget.dark ? Colors.white.withValues(alpha: 0.18) : Colors.black.withValues(alpha: 0.15),
                  ),
                ),
                alignment: Alignment.center,
                child: Text(
                  ...
                  style: TextStyle(
                    fontSize: 15,
                    fontWeight: FontWeight.w500,
                    color: widget.dark ? Colors.white.withValues(alpha: 0.55) : Colors.black.withValues(alpha: 0.5),
                  ),
                ),
              ),
      ),
...
class _FaqModalState extends State<FaqModal> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final qCol     = dark ? Colors.white.withValues(alpha: 0.88) : Colors.black.withValues(alpha: 0.85);
    final aCol     = dark ? Colors.white.withValues(alpha: 0.5)  : Colors.black.withValues(alpha: 0.5);
    final divCol   = dark ? Colors.white.withValues(alpha: 0.07) : Colors.black.withValues(alpha: 0.07);
            ...
            child: SlideTransition(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: dark ? kPurple.withValues(alpha: 0.3) : kCyan.withValues(alpha: 0.2),
                  ),
                ),
...
class _RedirectModalState extends State<RedirectModal> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final sub  = dark ? Colors.white.withValues(alpha: 0.5) : Colors.black.withValues(alpha: 0.5);
    final bor  = dark ? Colors.white.withValues(alpha: 0.1) : Colors.black.withValues(alpha: 0.12);
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            alignment: Alignment.center,
                            child: Text('Cancel',
                                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: sub)),
                          ),
                        ),
                      ),
                      const SizedBox(width: 12),
